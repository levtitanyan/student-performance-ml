...
def plot_missing_heatmap(df: pd.DataFrame) -> plt.Figure:
    """
    Create and return a clean transposed heatmap of missing values.
    ...
    sns.heatmap(
        ...
        cbar=False,
        cmap="Blues",
        linewidths=0,      # Remove grid effect     # Disable line color
    )

    ax.set_xticks([])
    ax.set_xticklabels([])

    ax.set_title("Missing Value Heatmap (Transposed)", fontsize=14, weight="bold")
...
def plot_numeric_distributions(df: pd.DataFrame, num_cols: list, n_cols: int = 4) -> plt.Figure:
    """
    Plot histograms for numerical columns in a grid layout.
